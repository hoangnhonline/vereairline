<?php
define('GUSER', 'vereairline.com@gmail.com');
define('GPWD', '123abcdef');
class Home {  
    ...
    function getCityById($city_id){
        echo $city_id;
        echo $sql = "SELECT city_name FROM city WHERE city_id = $city_id";
    ...
    function getListDeal(){
        $arrReturn = array();
        $sql = "SELECT * FROM product WHERE is_saleoff = 1 AND (deal_amount > 0 OR da_ban > 0 ) ORDER BY RAND() LIMIT 0,5";        
        $rs = mysql_query($sql) or die(mysql_error());
        while($row = mysql_fetch_assoc($rs)){
            $arrReturn[] = $row;
        }
    ...
    function getManuNameById($id){
        $name = "";
        $sql = mysql_query("SELECT manu_name FROM manu WHERE id = $id");
        $arrReturn = mysql_fetch_assoc($sql);        
        $name = $arrReturn['manu_name'];
        return $name;
    }
    ...
    function getMethodById($id){
     ...
     function insertOrderDetail($order_id,$product_id,$product_name,$color_id, $size_id, $for_id, $amount,$price,$total){
        $time = time();
        $sql = "INSERT INTO order_detail VALUES(NULL,$order_id,$product_id,'$product_name',$color_id, $size_id, $for_id, $amount,$price,$total,$time,1)";
        mysql_query($sql);
    }
    ...
    function getListProductNew($offset=-1,$limit=-1){
        $arrReturn = array();
        $sql = "SELECT * FROM product WHERE is_new = 1  ORDER BY id DESC ";
        if ($limit > 0 && $offset >= 0){
            $sql .= " LIMIT $offset,$limit";
        }        
        $rs = mysql_query($sql);
    ...
    function getListProductSearch($keyword,$color_id, $offset=-1,$limit=-1){
        $keyword = $this->processData($keyword);
        $arrReturn = array();
        $sql = "SELECT * FROM product" ;
            if($color_id > 0) $sql.=" ,product_color ";
            $sql.=" WHERE ( product_name LIKE '%".$keyword."%' ) OR ( product_code LIKE '%".$keyword."%' ) ";
            if($color_id > 0) $sql.=" AND product.id =  product_color.product_id AND (color_id = $color_id OR $color_id = -1 )";
        $sql .= " AND hidden = 0  ORDER BY id DESC ";    
        if ($limit > 0 && $offset >= 0){
            $sql .= " LIMIT $offset,$limit";
        }        
        $rs = mysql_query($sql);
    ...
    function getListProductSaleHot($offset=-1,$limit=-1){
        $arrReturn = array();
        $sql = "SELECT * FROM product WHERE sale_hot = 1  ORDER BY id DESC ";
        if ($limit > 0 && $offset >= 0){
            $sql .= " LIMIT $offset,$limit";
        }        
        $rs = mysql_query($sql);
    ...
    function getProductRelated($parent_cate, $product_id){        
        $arrReturn = array();
        $sql = "SELECT product.* FROM product,product_cate WHERE product_cate.cate_id = $parent_cate AND product_cate.product_id = product.id AND product.id <> $product_id ORDER BY product.id DESC LIMIT 0,5";
        $rs = mysql_query($sql);
        while($row = mysql_fetch_assoc($rs)){
            $arrReturn[$row['id']] = $row;
        }
    ...
    function phantrang($page, $page_show, $total_page, $link) {
        ...
        if($page < $total_page) {
            ($page==$total_page) ? $class = "class='active'" : $class="class='last'" ;
            echo "<li ".$class."><a href=" . $link . $pc ."trang=$total_page>Cuối</a></li>";
        }
        echo "</ul></div>";
    }
    ...
    function getDetailColor($color_id){
    ...
    function getListProductCate($parent_id=-1,$giatu=-1,$giaden=-1,$offset = -1, $limit = -1) {
        try{
            $arrResult = array();
            $sql = "SELECT product.* FROM product,product_cate WHERE (product_cate.cate_id = $parent_id OR $parent_id = -1) AND product_cate.product_id = product.id ";           
    ...
    function getListProductCateType($cate_type_id = -1,$hot = -1, $is_saleoff = -1, $offset = -1, $limit = -1) {
        try{
            $arrResult = array();
            $sql = "SELECT * FROM product WHERE (cate_type_id = $cate_type_id OR $cate_type_id = -1) AND (is_hot = $hot OR $hot = -1) AND (is_saleoff = $is_saleoff OR $is_saleoff = -1) ";                      
            $sql.=" AND hidden = 0 ORDER BY created_at DESC ";                      
            if ($limit > 0 && $offset >= 0)
                $sql .= " LIMIT $offset,$limit";           
            $rs = mysql_query($sql) or die(mysql_error());
